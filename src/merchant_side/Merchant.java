...
public class Merchant {
	 ...
	 */
	public Boolean recieveOrder(File signatureFile, AsymmetricKey pk,
			Collection<byte[]> encryptedOI, String pimd) {
		String dualSignature = decryptSignature(read(signatureFile), pk);
		String digestedOI = digestOI(encryptedOI);
		
		String paymentOrder = Digestor.process(pimd.concat(digestedOI));
	 ...
	 * @return
	 */
	private String digestOI(Collection<byte[]> cipherCollection) {
